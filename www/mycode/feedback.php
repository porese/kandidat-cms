...
if($sent_mail!=true) {
	echo '<form method="post" action="'.$url.'" enctype="multipart/form-data" name="phmailer" onsubmit="return Checkit(this);">';
	if($allowattach > 0){
		echo'<div>
...
echo '<div class="inpfrmname">'.__('Имя').':
</div>
...
if($use_subject_drop AND is_array($subjects)) {
	echo '<select name="emailsubject" size="1">';
	while(list($key,$val)=each($subjects)) {
		echo '<option value="'.htmlspecialchars(stripslashes($val)).'">'.htmlspecialchars(stripslashes($val)).'</option>';
	}
	echo '</select>';
}else{
	echo '<input name="emailsubject" type="text" size="30" value="'.stripslashes(htmlspecialchars($emailsubject)).'" />';
}
echo '</div>
<div class="inpfrmend"></div>
<div class="inpfrmname">'.__('Кто Вы').':</div>
<div class="inpfrmval">';
if($use_ltd_drop AND is_array($ltds)) {
	echo '<select name="ltd">';
	while(list($key,$val)=each($ltds)) {
		echo '<option value="'.htmlspecialchars(stripslashes($val)).'">'.htmlspecialchars(stripslashes($val)).'</option>';
	}
	echo '</select>';
}else{
	echo '<input name="ltd" type="text" size="30" value="'.stripslashes(htmlspecialchars($ltd)).'" />';
}
echo '</div><div class="inpfrmend"></div>';
for($i=1;$i <= $allowattach; $i++) {
	echo '<div class="inpfrmname">'.__('Прикрепить файл').':</div>
	<div class="inpfrmval"><input name="attachment[]" type="file" /></div>
	<div class="inpfrmend"></div>';
}
echo '<div class="inpfrmmessage">
	<label>'.__('Сообщение').':
...
echo '<div class="thanks_message">'.__('Спасибо').'! '.__('Ваше письмо было отправлено, мы ответим в ближайшее время').'</div>
